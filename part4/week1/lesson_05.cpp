...
struct Address {
    ...
    bool operator==(const Address& other) const {
        return this->city == other.city && this->street == other.street && this->building == other.building;
    }
};
...
struct Person {
    ...
    bool operator==(const Person& other) const {
        return this->name == other.name && this->height == other.height && this->weight == other.weight && this->address == other.address;
    }
};

struct AddressHasher {
    size_t operator()(const Address& value) const {
        const size_t Number = 63018038201;
        const hash<string> shash;
        return shash(value.city) * Number * Number + shash(value.street) * Number + hash<int>()(value.building);
    }
};
...
struct PersonHasher {
    size_t operator()(const Person& value) const {
        const size_t Number = 63018038201;
        return hash<string>()(value.name) * Number * Number * Number + hash<int>()(value.height) * Number * Number +
                hash<double>()(value.weight) * Number + AddressHasher()(value.address);
    }
};
