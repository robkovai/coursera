...
public:
    ...
    WriteAccess operator[](const K& key) {
        auto &bucket = buckets[hasher(key) % buckets.size()];
        unique_lock<mutex> lock(bucket.m);
        if (!bucket.values.count(key))
            bucket.values.insert({ key, V() });
        return { move(lock), bucket.values[key] };
    }
    ...
    ReadAccess At(const K& key) const {
        ...
        return { move(lock), bucket.values.at(key) };
    }

    bool Has(const K& key) const {
        auto &bucket = buckets[hasher(key) % buckets.size()];
        lock_guard lock(bucket.m);
        return bucket.values.count(key);
    }
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (auto& bucket : buckets) {
            lock_guard lock(bucket.m);
            result.insert(bucket.values.begin(), bucket.values.end());
        }
        return result;
    }
