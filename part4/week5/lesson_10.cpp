...
public:
    Budget() {
        this->data.resize(Date::getMaxDay());
    }

    double computeIncome(Date from, Date to) const {
        return accumulate(this->getIncomeIterator(from), next(this->getIncomeIterator(to)), 0.0, [](double sum, const Item& item) {
            return sum += item.income - item.spend;
        });
    }
    ...
    void earn(Date from, Date to, double value) {
        ...
        for (size_t i = from.getEpochDays(); i < to.getEpochDays() + 1; ++i)
            this->data[i].income += cost;
    }

    void spend(Date from, Date to, double value) {
        auto cost = value / (to.getEpochDays() - from.getEpochDays() + 1);
        for (size_t i = from.getEpochDays(); i < to.getEpochDays() + 1; ++i)
            this->data[i].spend += cost;
    }

    void payTax(Date from, Date to, double value) {
        for (size_t i = from.getEpochDays(); i < to.getEpochDays() + 1; ++i)
            this->data[i].income *= 1.0 - value / 100.0;
    }

private:
    Data::const_iterator getIncomeIterator(const Date& date) const {
        return next(this->data.begin(), min(date.getEpochDays(), this->data.size()));
    }
...
void TestBudget() {
    ...
    budget.payTax(Date(2000, 1, 2), Date(2000, 1, 3), 13);
    ASSERT_EQUAL(budget.computeIncome(Date(2000, 1, 1), Date(2001, 1, 1)), 18.96);

    budget.spend(Date(2000, 12, 30), Date(2001, 1, 2), 14);
    ASSERT_EQUAL(budget.computeIncome(Date(2000, 1, 1), Date(2001, 1, 1)), 8.46);

    budget.payTax(Date(2000, 12, 30), Date(2000, 12, 30), 13);
    ASSERT_EQUAL(budget.computeIncome(Date(2000, 1, 1), Date(2001, 1, 1)), 8.46);
}
