...
void TestSplit() {
    ASSERT_EQUAL(Split("a b c"), vector<string_view>({"a", "b", "c"}));
}

void TestGetReversedParts() {
    Domain domain("a.b.c");
    auto parts = domain.GetReversedParts();
    auto result = vector<string>(parts.begin(), parts.end());
    ASSERT_EQUAL(result, vector<string>({"c", "b", "a"}));
}

void TestSubdomain() {
    Domain domain("b.c");
    Domain subdomain("a.b.c");
...
void TestDomainChecker() {
    vector<Domain> domains = {Domain("a.b.c"), Domain("b.c")};
    ASSERT_EQUAL(DomainChecker(domains.begin(), domains.end()).IsSubdomain(Domain("z.b.c")), true);
}

void TestCheckDomain() {
    ASSERT_EQUAL(CheckDomains(vector<Domain>({Domain("b.a")}), vector<Domain>({Domain("c.b.a")})), vector<bool>({false}));
}

void TestPrintCheckResults() {
    stringstream ss;
    PrintCheckResults(vector<bool>({false, true}), ss);
    ASSERT_EQUAL(ss.str(), "Bad\nGood\n");
}

void TestReadDomains() {
    stringstream ss("1\na.b");
    ASSERT_EQUAL(ReadDomains(ss), vector<Domain>({Domain("a.b")}));
}
