...
public:
    ...
    size_t Get(TAirport airport) const {
        return data[toSizeT(airport)].second;
    }

    void Insert(TAirport airport) {
        data[toSizeT(airport)].first = airport;
        data[toSizeT(airport)].second++;
    }
...
private:
    void init() {
        for (size_t i = 0; i < toSizeT(TAirport::Last_); ++i)
            data[i].first = static_cast<TAirport>(i);
    }

    size_t toSizeT(TAirport airport) const {
